...
def count_plank_pose():
    ...
    try:
        while time.time() - start_time < timeout:
            ...
            if results.pose_landmarks:
                ...
                # Check if in plank pose
                if (abs(left_arm_angle) < 30 and abs(right_arm_angle) < 30 and 
                    abs(left_leg_angle) < 30 and abs(right_leg_angle) < 30):
                    plank_pose_count += 1
                    time.sleep(1)  # Wait for 1 second to avoid counting the same pose multiple times
                
                # Break if we've counted 10 poses
                if plank_pose_count >= 10:
                    break
    ...
    return f"Completed {plank_pose_count} plank poses" 